...
TextureMap::TextureMap()
{
 ...
 TextureMap::TextureMap(const std::string& fileName , const std::string& type)
{
	...
	if (type == "normal")
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, imageData); //load image data into texture
	}
	else
	{
...
void TextureMap::LoadTexture(const std::string& fileName, const std::string& type)
{
	int width, height, numComponents; //width, height, and no of components of image
	unsigned char* imageData = stbi_load((fileName).c_str(), &width, &height, &numComponents, 4); //load the image and store the data
